...
class GameScene2 extends Phaser.Scene {
    ...
    preload() {
        this.load.image('psyduck', 'assets/segundaFase/psyduck.png');
        this.load.image('concha', 'assets/segundaFase/concha.png');
        this.load.image('magikarp', 'assets/segundaFase/magikarp.png');
        this.load.image('caverna', 'assets/segundaFase/caverna.png');
        this.load.tilemapTiledJSON('map2', 'assets/map2.json');
        this.load.image('tiless', 'assets/tileset.png');
        this.load.image('backgroundGame2', 'assets/segundaFase/background2.jpg');
    }

    create() {
        this.add.image(400, 300, 'backgroundGame2');
        const map = this.make.tilemap({ key: "map" });
        const tilesset = map.addTilesetImage("tilesset", "tiless");
        map.createLayer("Ground", tilesset, 0, 0);
        
        this.player = this.physics.add.sprite(100, 100, 'psyduck').setScale(0.3);
        this.player.setCollideWorldBounds(true);
        this.player.setBounce(0.2);
        
        this.spawnKey();
        this.door = this.physics.add.sprite(500, 200, 'caverna').setScale(0.5);
        this.physics.add.overlap(this.player, this.door, this.enterDoor, null, this);
        
        this.enemy = this.physics.add.sprite(400, 200, 'magikarp').setScale(0.1);
        this.enemy.setVelocity(100, 100);
        this.enemy.setBounce(1, 1);
        this.enemy.setCollideWorldBounds(true);
    ...
    spawnKey() {
        ...
        let x = Phaser.Math.Between(50, 750);
        let y = Phaser.Math.Between(50, 550);
        this.keyItem = this.physics.add.sprite(x, y, 'concha').setScale(0.1);
        this.physics.add.overlap(this.player, this.keyItem, this.collectKey, null, this);
        this.hasKey = false;
    }
